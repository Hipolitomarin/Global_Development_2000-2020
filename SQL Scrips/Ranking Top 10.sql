...
-- Normalización con rangos por columna ignorando NULL
normalized AS (
    SELECT *,
...
scored AS (
    SELECT *,
        -- Contar cuántos indicadores tiene
        (
        ...
        -- Calcular promedio solo de los disponibles
        ROUND((
            COALESCE(norm_gdp, 0) +
            COALESCE(norm_env, 0) +
...
SELECT
    country_name,
    global_score,
    RANK() OVER (ORDER BY global_score DESC) AS ranking
FROM scored
WHERE non_null_count >= 3  -- Al menos 3 indicadores para ser considerado
ORDER BY ranking
LIMIT 10;